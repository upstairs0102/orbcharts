...
> **Tip**: The default color source is `series` (`color: { from: 'series' }`). That is, records with the same `series` share the same color. You can switch to coloring by `category` or `index`.
