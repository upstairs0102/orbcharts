...
|---|---|---|---|
| `rankedScale` | `{ label: string, limit: number \| 'auto' }` | `{ label: '', limit: 10 }` | Rank axis settings; `limit` is the maximum number of ranks to show |
...
interface RankedPlotPluginParams {
  ...
  rankedScale: { label: string; limit: number | 'auto' }
  datasetIndex: number
}
