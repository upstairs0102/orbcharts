...
|---|---|---|---|
| `rankedAxis` | `{ label: string, limit: number \| 'auto' }` | `{ label: '', limit: 10 }` | Rank axis settings; `limit` is the maximum number of ranks to show |
| `categoryAxis` | `CategoryAxis` | see [CategoryAxis](#categoryaxis) | Category axis settings |
...
| `seriesLabel.colorType` | `ColorType` | `'primary'` | Series label color |
...
interface RankedPlotPluginParams {
  ...
  rankedAxis: { label: string; limit: number | 'auto' }
  datasetIndex: number
}
