...
|---|---|---|---|
| `rankedAxis` | `{ label: string, limit: number \| 'auto' }` | `{ label: '', limit: 10 }` | Ranking axis settings: `label` is the axis label, `limit` is the max number of ranks shown (`'auto'` for automatic) |
...
| `format` | `string \| ((text) => string)` | `text => text` | Value text formatter |

### SeriesLabel

| Parameter | Type | Default | Description |
|---|---|---|---|
| `axisLabel.offset` | `[number, number]` | `[0, 0]` | Axis label offset |
| `axisLabel.colorType` | `ColorType` | `'primary'` | Axis label color |
| `seriesLabel.position` | `'inside-left' \| 'inside-right' \| 'outside'` | `'inside-right'` | Series label position |
| `seriesLabel.padding` | `number` | `20` | Series label padding |
| `seriesLabel.colorType` | `ColorType` | `'dataContrast'` | Series label text color |
