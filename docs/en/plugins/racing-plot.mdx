...
|---|---|---|---|
| `rankedScale` | `{ label: string, limit: number \| 'auto' }` | `{ label: '', limit: 10 }` | Ranking axis settings: `label` is the axis label, `limit` is the max number of ranks shown (`'auto'` for automatic) |
